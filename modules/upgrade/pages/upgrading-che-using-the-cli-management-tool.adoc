...
* You have an administrative account on {platforms-name}.

* You have a running instance of a previous minor version of {prod-prev-short}, installed using the CLI management tool on the same instance of {platforms-name}, in the `{prod-namespace}` {platforms-namespace}.

* You have `{prod-cli}` for {prod-short} version {prod-ver}. See: xref:install:installing-the-chectl-management-tool.adoc[].
